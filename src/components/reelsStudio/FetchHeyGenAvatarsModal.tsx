            ...
            <>
              <div className="flex items-center justify-between mb-3">
                <button
                  ...
                  className="text-[11px] px-2 py-1 rounded-md bg-violet-500/15 hover:bg-violet-500/25 border border-violet-500/30 text-violet-200 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                >
              ...
              <div className="grid grid-cols-3 gap-2">
                {remote.map(r => {
                  ...
                  return (
                    ...
                    >
                      <div className="absolute inset-0 bg-gradient-to-br from-violet-500/20 to-violet-700/20 flex items-center justify-center">
                        <span className="text-2xl font-bold text-violet-200">{r.name.slice(0, 1).toUpperCase()}</span>
                      </div>
                      {r.previewUrl && (
                        <img
                          src={r.previewUrl}
          ...
          {importing ? (
            <div className="space-y-2">
              ...
              <div className="h-1 bg-white/5 rounded-full overflow-hidden">
                <div className="h-full bg-gradient-to-r from-violet-500 to-violet-400 transition-all" style={{ width: `${(importProgress / Math.max(selected.size, 1)) * 100}%` }}></div>
              </div>
            </div>
          ) : (
