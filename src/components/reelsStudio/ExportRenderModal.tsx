      ...
      const handle = renderMp4(
        {
          ...
          // Edit-video mode: the uploaded video is the full-frame base track.
          baseVideoTake: state.projectMode === 'edit' ? (state.baseVideoTake ?? null) : null,
          overlayElements: state.projectMode === 'edit' ? (state.overlayElements ?? []) : undefined,
          audioBlob,
    ...
    return (
      <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-[80] p-6">
        <div className="bg-[#141416] border border-white/10 rounded-2xl shadow-2xl max-w-md w-full p-6">
          ...
          <div className="rounded-lg bg-blue-500/[0.05] border border-blue-500/20 p-3 mb-4">
          ...
          {readyToRender && (
            <>
              <div className="px-6 pb-4 space-y-3">
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      ...
                      className={`px-3 py-3 rounded-lg border text-left transition-colors ${quality === 'high' ? 'bg-blue-500/15 border-blue-400/50' : 'bg-black/20 border-white/10'}`}
                    >
                    ...
                    <button
                      ...
                      className={`px-3 py-3 rounded-lg border text-left transition-colors ${quality === 'lite' ? 'bg-blue-500/15 border-blue-400/50' : 'bg-black/20 border-white/10'}`}
                    >
                ...
                <div className="rounded-lg bg-black/30 border border-white/5 p-3 space-y-1.5 text-xs">
                  ...
                  {state.audio.silenceCut && state.audio.detectedSilenceSec >= 0.1 && (
                    <div className="flex justify-between"><span className="text-zinc-500">Cortes de silêncio</span><span className="text-blue-300">✂ −{state.audio.detectedSilenceSec.toFixed(1)}s ({state.audio.silencePreset})</span></div>
                  )}
              ...
              <div className="px-6 py-4 flex gap-2 border-t border-white/5 bg-black/30">
                ...
                <button onClick={startRender} className="flex-1 py-2.5 rounded-lg bg-gradient-to-b from-blue-500 to-blue-600 hover:from-blue-400 hover:to-blue-500 text-xs font-semibold text-white shadow-[0_0_20px_rgba(10,132,255,0.5)]">
                  Renderizar agora
                </button>
              </div>
            </>
    ...
    return (
      <div className="fixed inset-0 bg-black/95 backdrop-blur-2xl flex items-center justify-center z-[90]">
        <div className="text-center max-w-md w-full px-6">
          <div className="text-[10px] uppercase tracking-widest text-blue-300/70 font-semibold mb-4">Renderizando seu reel</div>
          ...
          <div className="mb-3">
            <div className="h-2 rounded-full bg-white/5 overflow-hidden">
              <div className="h-full bg-gradient-to-r from-blue-500 to-blue-400 transition-all" style={{ width: `${pct}%` }}></div>
            </div>
      ...
      <div className="fixed inset-0 bg-black/85 backdrop-blur-xl flex items-center justify-center z-[80] p-6" onClick={onClose}>
        <div className="bg-[#141416] border border-white/10 rounded-2xl shadow-2xl max-w-md w-full overflow-hidden" onClick={e => e.stopPropagation()}>
          ...
          <div className="px-6 py-4 flex gap-2 border-t border-white/5">
            ...
            <button onClick={downloadResult} className="flex-1 py-2.5 rounded-lg bg-gradient-to-b from-blue-500 to-blue-600 hover:from-blue-400 hover:to-blue-500 text-xs font-semibold text-white shadow-[0_0_20px_rgba(10,132,255,0.5)] flex items-center justify-center gap-2">
      ...
      <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-[80] p-6">
        <div className="bg-[#141416] border border-red-500/30 rounded-2xl shadow-2xl max-w-md w-full p-6">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setPhase('config')} className="flex-1 py-2.5 rounded-lg bg-blue-500 hover:bg-blue-400 text-xs font-semibold text-white">Tentar de novo</button>
          </div>
        </div>
      </div>
    );
